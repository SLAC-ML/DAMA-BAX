...
from .dama_resources import get_sextupole_setup, get_init_config, get_ring_file, get_setup_file
...
def make_gen_mamap():
    setup = np.load(str(get_setup_file()))
...
# Load the variables
data = loadmat(str(get_sextupole_setup()))
SextPara = data['SextPara'][0]
g_dpp_index = data['g_dpp_index']

data = loadmat(str(get_init_config()))
...
# Load the rings (only those that exist)
rings = []
with open(str(get_ring_file(0)), "rb") as f:
    rings.append(pickle.load(f))
for i in range(1, 26):  # Changed from 27 to 26 (ring_s26.pkl doesn't exist)
    try:
        with open(str(get_ring_file(i)), "rb") as f:
            rings.append(pickle.load(f))
    except FileNotFoundError:
        print(f"Warning: ring_s{i}.pkl not found, skipping")
        continue
